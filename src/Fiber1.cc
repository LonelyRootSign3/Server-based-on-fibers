...
Fiber::Fiber(bool use_main,std::function<void()> sc_cb,int size)
    :m_fib_id(++s_fiber_id){
        ...
        m_ctx.uc_link = nullptr;//协程框架设计里不用uclink
...
void Fiber::SwapOut(){
    SetCurFiber(t_schedule_fiber);
    if(swapcontext(&m_ctx,&(t_schedule_fiber->m_ctx))){
        MY_ASSERT2(false,"swapcontext");
    }
}
...
void Fiber::RootScheduleSwapOut(){
    SetCurFiber(t_root_fiber);
    if(swapcontext(&m_ctx,&(t_root_fiber->m_ctx))){
        MY_ASSERT2(false,"swapcontext");
    }
}
...
void Fiber::ContextFunc(){
    ...
    t_curfiber->SwapOut();
    MY_ASSERT2(false,"never reach fiber id : "+std::to_string(t_curfiber->m_fib_id));
}
...
void Fiber::RootScheduleContextFunc(){
    ...
    t_schedule_fiber->RootScheduleSwapOut();

    MY_ASSERT2(false,"never reach root schedule fiber id : "+std::to_string(t_schedule_fiber->m_fib_id));
